...
theory Tests_Parametric_With_Casts_Instantiation_Word_Lemmas_Internal
imports Word_Lemmas More_Word_Operations Many_More Word_Syntax Syntax_Bundles
begin

(* Comment from authors: "this bundle doesn't survive theory merges from HOL.Main, because these 
   simp/split rules are added there, so it may need to be unbundled or included locally more than 
   once"
*)
...
(* Comment from authors: "Override default word enum code generation setup, so that "value" and "eval"
...
(* NOTE: I left this lemmas as it is needed to prove the of_nat_shift_distinct_helper *)
...
(* NOTE: included since it depends on a lemma that uses instantiation *)
...
(* NOTE: This lemma is required to prove lemma mask_shiftl_le_mask *)
...
(* NOTE: required for sint_ucast_uint_pred *)
...
(* NOTE: required for a lemma, below *)
...
(* NOTE: required for  word_wrap_of_natD *)
lemma unat_minus':
  fixes x :: "'a :: len word"
...
(* NOTE: this lemma is included since it depends on lemmas that use casting *)
...
(* NOTE: the following def. and lemmas are required for word_clz_shiftr_1 *)
...
(* NOTE: required for takeWhile_truncate *)
...
(* NOTE: this is required to prove word_clz_shiftr *)
lemma shiftr_Suc:
  fixes x::"'a::len word"
  shows "x >> (Suc n) = x >> n >> 1"
...
(* NOTE: dependency of a lemma, below *)
...
(* NOTE: dependency of another lemma *)
...
(* NOTE: depends on lemmas with instantiation *)
...
*)
(* NOTE: this lemma depends on a lemma instantiated with "of" *)
