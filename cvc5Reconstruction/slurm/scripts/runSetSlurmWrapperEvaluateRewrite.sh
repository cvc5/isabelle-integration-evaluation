...
while read -r problem_file; do
  ...
  new=$base_dir/$input_file/$temp
  rewrite=$(cat $new | grep "rare_rewrite" | sed 's/.*rare_rewrite :args ("//g' | sed 's/".*$//g')

  temp=${temp%.*}.smt2
  
  entry=$(echo $entry | sed 's|benchmark_path": "[^,]*", |benchmark_path": \"'"$entry2"'\", \"rewrite\": \"'"$rewrite"'\", "library_name": \"'"$library"'\","benchmark_steps": '"$steps"', |' )
