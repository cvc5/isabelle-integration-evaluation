...
Help()
{
   ...
   echo "Usage: $0 <input directory ABSOLUTE path> <script name (needs to be in holes dir)>"
   echo "Available scripts are: findHoles.sh"
   echo
   echo "options:"
...
if [[ "$#" -ne 2 ]]; then
  echo "Usage: $0 [options] <input directory ABSOLUTE path> <script name (needs to be in holes dir)>"
  exit 1
fi
if ! [[ "$INPUT_DIR" =~ ^/ ]]; then
...
find "$INPUT_DIR" -type f -name "*.alethe" > $benchmark_set_slurm 
